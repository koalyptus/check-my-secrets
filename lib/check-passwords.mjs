...
export async function checkPasswords(passwords = []) {
  ...
  for (let password of passwords) {
    ...
    if (isCompromised) {
      ...
      detailsCollection = detailsCollection.concat(`'${masked}'\n`);
  ...
  if (nbSkipped > 0) {
    return {
      ...
      message:
        `Checked ${passwords.length} passwords, ${nbSkipped} skipped, ${nbCompromised} compromised` +
        (nbCompromised > 0
          ? `:
${details}`
          : `.`)
...
async function checkHash(hash = '', timeoutMs = 10000) {
